...
import { useMapFeaturePointerPress } from "./use-map-feature-pointer-press";
...
export function MapCanvas({
  ...
  const interactiveLayerIds = useMemo(
    ...
    [extraInteractiveLayerIds, onMarkerPress, onUserLocationPress],
  );

  const featurePressHandlers = useMemo<MapFeaturePressHandlers>(
    () => ({
      onMarkerPress,
      extraInteractiveLayerIds,
      onLayerFeaturePress,
  ...
  const featurePointerPress = useMapFeaturePointerPress(
    mapRefHolder,
    interactiveLayerIds,
    featurePressHandlers,
  );

  const handleClick = useCallback(
    (event: MapMouseEvent) => {
      featurePointerPress.notifyMapClickHandled();
      resolveMapFeaturePress(event.features ?? [], featurePressHandlers);
    },
    [featurePointerPress, featurePressHandlers],
  );
  ...
  return (
    <div
      ...
      onPointerDown={featurePointerPress.onPointerDown}
      onPointerUp={featurePointerPress.onPointerUp}
    >
      <GlMap
        ref={handleMapRef}
